...
class TextLintModuleLoader extends events_1.EventEmitter {
    ...
    loadFromConfig(config) {
        ...
        if (config.rulePaths && this.listenerCount(TextLintModuleLoader.Event.rule) > 0) {
            ...
            config.rulePaths.forEach((rulesDir) => {
                ...
                const rules = rule_loader_1.loadFromDir(rulesDir);
    ...
    loadPlugin(pluginName) {
        ...
        const plugin = module_interop_1.moduleInterop(require(pkgPath));
        const pluginNameWithoutPrefix = utils_1.normalizeTextlintPluginKey(pluginName);
...
For more details. See https://github.com/textlint/textlint/blob/master/docs/plugin.md`);
    ...
    loadPreset(presetName) {
         ...
         */
        const presetRuleNameWithoutPrefix = utils_1.normalizeTextlintRulePresetKey(presetName);
        ...
        const preset = module_interop_1.moduleInterop(require(pkgPath));
    ...
    loadRule(ruleName) {
        ...
        */
        ...
        if (isFile(ruleName)) {
            const ruleCreator = module_interop_1.moduleInterop(require(ruleName));
        ...
        const definedRuleName = utils_1.normalizeTextlintRuleKey(ruleName);
        // ignore plugin's rule
        ...
        const ruleCreator = module_interop_1.moduleInterop(require(pkgPath));
    ...
    loadFilterRule(ruleName) {
        ...
        */
        ...
        const definedRuleName = utils_1.normalizeTextlintFilterRuleKey(ruleName);
        // ignore plugin's rule
        if (config_util_1.isPluginRuleKey(definedRuleName)) {
            logger_1.Logger.warn(`${definedRuleName} is Plugin's rule. This is unknown case, please report issue.`);
            return;
